...
export class JournalController {
  ...
  async getEntries(req: AuthRequest, res: Response) {
    ...
    // Visibility rules:
    // 1. Public entries (isPrivate: false) - visible to all family members
    // 2. Private entries authored by current user - visible to author
    // 3. CeeCee conversation logs with visibleToUserIds containing current user
    where.OR = [
      ...
      {
        // CeeCee conversation logs shared with this user via visibleToUserIds
        AND: [
          { isPrivate: true },
  ...
  async getEntry(req: AuthRequest, res: Response) {
    ...
    // Private entries are visible to:
    // 1. The author
    // 2. Users listed in visibleToUserIds (for CeeCee conversation logs)
    if (entry.isPrivate && entry.userId !== user.id) {
      const isInVisibleList = entry.visibleToUserIds?.includes(user.id);
      if (!isInVisibleList) {
        throw new ApiError(ErrorCodes.FORBIDDEN, 'Private entries can only be viewed by their author or designated users', 403);
      }
    }
